...
#These lines establish where autosnort was executed. The config file _should_ be in this directory. the script exits if the config isn't in the same directory as the autosnort-debian shell script.
...
if [ $? -eq 0 ];then
	apt-key list | grep "dotdeb.org" &>> $logfile
	if [ $? -eq 0 ];then
...
print_status "Installing base packages: ethtool build-essential zlib1g-dev libpcap-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 libarchive-tar-perl libcrypt-ssleay-perl libwww-perl.."

declare -a packages=( ethtool build-essential zlib1g-dev libpcap-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 libarchive-tar-perl libcrypt-ssleay-perl libwww-perl );
...
case 1 in
	...
	#This is here for failed runs of autosnort. It turns out multiple runs of the scripts appends the redirect virtual host (below) multiple times to the default site vhost, which breaks things and makes httpd sad. If the backup file we make for the stock httpd.conf is here, we assume there was a failed autosnort install and copy over it, but not before saving httpd.conf (in case the user did something to it)
	
	if [ -f /etc/apache2/defaultsiteconfbak ]; then
		print_status "found /etc/apache2/defaultsiteconfbak. Making copy of current httpd.conf (/etc/apache2/sites-available/defaultsite_copy) and restoring original from /etc/apache2/defaultsiteconfbak.."
		cp /etc/apache2/sites-available/default /etc/apache2/sites-available/defaultsite_copy
		mv /etc/apache2/defaultsiteconfbak /etc/apache2/sites-available/default
	fi
	
	#If these files exist move/copy them as necessary. We copy the default vhost files so there's a backup available in case the user wants to remove the web IDS interface in the future (or there's a failed autosnort install and we don't make repeated edits cause the apache to fail entirely)
...
# note: Debian 6 doesn't seem to like snort.org's certificate... so --no-check-certificate was added as a work-around for debian 6 users.
...
if [[ $release == "6."* ]]; then
	wget http://www.snort.org/downloads/snort/$daqtar -O $daqtar --no-check-certificate &>> $logfile
...
if [[ $release == "6."* ]]; then
	wget http://www.snort.org/downloads/snort/$snorttar -O $snorttar --no-check-certificate &>> $logfile
...
case $ui_choice in
	...
	bash autosnortreport-debian.sh
	...
	bash autosyslog_full-debian.sh
